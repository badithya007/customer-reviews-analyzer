...
st.sidebar.markdown("<p style='text-align: center; font-size: 0.8rem; color: #94A3B8;'>Enterprise Feedback Optimization System</p>", unsafe_allow_html=True)
st.sidebar.markdown("---")
...
# Title Block
st.markdown("<h1 style='margin-bottom: 0px;'>📊 Enterprise Product Intelligence Console</h1>", unsafe_allow_html=True)
st.markdown("<p style='color: #94A3B8; font-size: 1.1rem; margin-bottom: 30px;'>Continuous Natural Language Analysis & Strategic Action Suite</p>", unsafe_allow_html=True)
...
if app_mode == "📊 Data Deck & Analytics":
    ...
    with col_meta:
        ...
        st.markdown(f"<span style='color:#8B5CF6; font-size:1.4rem; font-weight:bold;'>{st.session_state.product_category}</span>", unsafe_allow_html=True)
        st.write("")
    ...
    churn_risk_penalty = neg_ratio * 15  
    final_psr = max(min(round(raw_psr - churn_risk_penalty, 1), 100), 0)

    # Layout Metrics
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(f"<div class='metric-card'><p style='color:#94A3B8; margin:0;'>Total Volume</p><h2>{total_reviews} Records</h2></div>", unsafe_allow_html=True)
    with c2:
        st.markdown(f"<div class='metric-card'><p style='color:#34D399; margin:0;'>Positive Feedback</p><h2 style='color:#34D399 !important;'>{pos_count} ({round(pos_ratio*100)}%)</h2></div>", unsafe_allow_html=True)
    with c3:
    ...
    with c4:
        st.markdown(f"<div class='metric-card'><p style='color:#60A5FA; margin:0;'>Aggregated CSAT Score</p><h2>{round((avg_score/5)*100, 1)}%</h2></div>", unsafe_allow_html=True)
    ...
    st.markdown(f"Current Status: {verdict_badge}", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:1.1rem; margin-top:15px; color:#CBD5E1;'>{verdict_text}</p>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    for ticket in tickets:
        with st.expander(f"📥 Ticket Alert for: **{ticket['Dept']}** — {ticket['Priority']}"):
            st.markdown(f"<p style='font-size:1.05rem;'><b>Prescriptive Strategy Formulation:</b> {ticket['Suggestion']}</p>", unsafe_allow_html=True)